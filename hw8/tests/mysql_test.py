...
RESULTS_FILE = "../results/mysql_test_results.json"
...
print("Starting MySQL test - 150 operations (50 create, 50 add, 50 get)")

# 50 create cart
print("Running 50 POST /shopping-carts...")
for i in range(50):
    start = time.time()
...
# 50 add items
print("Running 50 POST /shopping-carts/:id/items...")
for i in range(50):
...
# 50 get cart
print("Running 50 GET /shopping-carts/:id...")
for i in range(50):
    cart_id = cart_ids[i % len(cart_ids)]
    start = time.time()
    r = requests.get(f"{BASE_URL}/shopping-carts/{cart_id}", timeout=30)
